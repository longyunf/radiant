...
def test(args, model, test_loader, epoch=None):
    ...
    loss_dict = dict(loss=0, loss_cls=0, loss_offset=0, loss_depth=0, loss_size=0, 
                     loss_rotsin=0, loss_bbox2d=0, loss_consistency=0, loss_centerness=0, 
                     loss_attr=0, loss_velo=0, loss_dir=0, loss_radarClass=0, 
                     loss_radarOffset=0, loss_radarDepthOffset=0) 
...
def init_params(args, model, optimizer):
    ...
    if args.load_pretrained_pgd and start_epoch==1:
        ...
        _ = load_checkpoint(model, args.path_checkpoint_pgd, map_location='cpu', \
...
def main(args):
    ...
    if args.path_checkpoint_pgd == None:
        this_dir = os.path.dirname(__file__)
        args.path_checkpoint_pgd = join(this_dir, '..', 'checkpoints', 'pgd_r101_caffe_fpn_gn-head_2x16_2x_nus-mono3d_finetune_20211114_162135-5ec7c1cd.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_checkpoint_pgd', type=str) 
    ...
    parser.add_argument('--eval_mono', action='store_true', default=False, help='compute mAP for camera outputs')
